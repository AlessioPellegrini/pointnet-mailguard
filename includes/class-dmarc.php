...
class PN_Mailguard_DMARC {
    ...
    public static function analyze(string $input): array {
        ...
        foreach ($txt_records as $rec) {
            if (!empty($rec['txt']) && str_starts_with(trim($rec['txt']), 'v=DMARC1')) {
                $record = trim($rec['txt']);
                break;
            }
        }
        ...
        if (empty($tags['sp'])) {
            ...
            if (in_array($sp, ['reject', 'quarantine'], true)) {
        ...
        $policy_sev = match (true) {
            $spf_status === 'missing'                                                             => 'error',
            $spf_status === 'error'                                                               => 'warning',
            $policy === 'reject' && $spf_status === 'warning'                                     => 'warning',
            default                                                                               => 'ok',
        };
        $spf_msgs = [
            'error'   => ['SPF record missing — DMARC effectiveness reduced', 'DMARC works best when both SPF and DKIM are configured. Your domain has no SPF record — fix this to maximise DMARC protection.'],
            'warning' => ['SPF record has errors — DMARC effectiveness reduced', 'Your SPF record has configuration errors. Fix SPF to ensure DMARC can use it for authentication.'],
            'ok'      => ['SPF and DMARC are consistent', 'Your SPF record is correctly configured and consistent with your DMARC policy.'],
        ];
        if ($policy === 'reject' && $spf_status === 'warning') {
            $spf_msgs['warning'] = ['DMARC policy is strict but SPF has warnings', 'You have p=reject but your SPF record has warnings (e.g. ~all or ?all). For maximum effectiveness, align your SPF to use -all.'];
        }
        [$spf_title, $spf_desc] = $spf_msgs[$policy_sev];
        $checks[] = self::result('spf_correlation', $policy_sev, $spf_title, $spf_desc);
        match ($policy_sev) {
            'error'   => $errors++,
            'warning' => $warnings++,
            default   => $passed++,
