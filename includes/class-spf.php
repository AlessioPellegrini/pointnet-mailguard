...
class PN_Mailguard_SPF {
    ...
    public static function analyze(string $input): array {
        ...
        foreach ($txt_records as $rec) {
            if (!empty($rec['txt']) && str_starts_with(trim($rec['txt']), 'v=spf1')) {
        ...
        foreach ($mechanisms as $mech) {
            ...
            if (preg_match('/^[+\-~?]?(include):(.+)$/i', $mech, $m)) {
                ...
                foreach (self::$providers as $pattern => $name) {
                    if (str_contains($inc, $pattern)) {
                        if (!in_array($name, $providers, true)) $providers[] = $name;
                        break;
                    }
                }
     ...
     * Triggers alert only for real problems (missing, error, or +all),
     * not for mere recommendations (~all or ?all).
     */
    public static function check(string $domain): array {
        ...
        // Only trigger alert for missing/error, not for recommendations like ~all
